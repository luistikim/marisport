...
export function ProductCard({ product, compact = false }: ProductCardProps) {
  ...
  if (compact) {
    return (
      <article className="group overflow-hidden rounded-[1.2rem] border border-[#d9e5dc] bg-white shadow-[0_10px_26px_rgba(19,38,59,0.06)] transition-[box-shadow,transform] duration-300 ease-out hover:-translate-y-0.5 hover:shadow-[0_18px_42px_rgba(19,38,59,0.1)] sm:rounded-[1.5rem]">
        <Link href={detailHref} aria-label={`Ver detalhes de ${product.name}`} className="block">
              ...
              <Image
                ...
                className={
                  product.imageFit === "contain"
                    ? "object-contain p-2.5 transition-transform duration-500 ease-out group-hover:scale-[1.05]"
          ...
          <div className="space-y-2 p-3 sm:p-4">
            <div className="flex items-center justify-between gap-2">
              <h3 className="min-w-0 flex-1 text-[0.92rem] font-semibold leading-snug text-surface-strong sm:text-[1rem]">
                {product.name}
              </h3>
              {product.statusLabel ? (
                <span className="shrink-0 rounded-full bg-[#f1f6f1] px-2 py-1 text-[9px] font-bold uppercase tracking-[0.14em] text-[#4c6454]">
                  {product.statusLabel}
                </span>
              ) : null}
            </div>
            ...
            <div className="flex items-end justify-between gap-2">
              ...
              {product.originalPrice && hasPrice && price !== null ? (
                <p className="text-[10px] font-bold uppercase tracking-[0.12em] text-[#a2aca5] line-through">
